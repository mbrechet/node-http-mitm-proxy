...
// forward cookies
request = request.defaults({jar:true,pool:{maxSockets:Infinity}});
...
Proxy.prototype.listen = function(options) {
  ...
  mkdirps(this.sslCaDir, function(err) {
    ...
    self.httpServer = options.httpServer || http.createServer();
...
Proxy.prototype._onError = function(kind, ctx, err) {
  ...
  if (ctx) {
    ...
    // remove this to prevent unexpected  end of socket
...
Proxy.prototype._onHttpServerRequest = function(isSSL, clientToProxyRequest, proxyToClientResponse) {
  ...
  ctx.proxyToServerRequestOptions = {
    ...
    url: ctx.isSSL ? 'https://'+ctx.clientToProxyRequest.headers.host + ctx.clientToProxyRequest.url : ctx.clientToProxyRequest.url,
  ...
  function makeProxyToServerRequest() {
    ...
    ctx.proxyToServerRequest = request(ctx.proxyToServerRequestOptions);
    ctx.proxyToServerRequest.on('response',proxyToServerRequestComplete);
